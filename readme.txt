...
Note: Please see the FAQ section for an important note about the Dow Jones
Industrial Average.
...
= Why doesn't the Dow Jones Industrial Average (^DJI or INDU) display? =

Yahoo no longer provides CSV/XML download of the DJIA, due to restrictions
imposed by Dow Jones.  Since the plugin relies on CSV data, there is
currently no way to display this average.
